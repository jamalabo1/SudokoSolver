...
<html lang='en'>
  <head>
    ...
    <title>Jamal Abo Mokh Sudoku mfs</title>
  ...
  <body>
    ...
    <div class='container'>
      ...
      <h1>Sudoku Solver v2</h1>

      <table id='grid'>

         ";

        private const string Footer = @"</table>
        ...
        private static string ToReadableString(IEnumerable<int> numbers)
        {
        ...
        public static string GenerateHtmlFileFromBoard(Board board)
        {
            ...
            for (var line = 1; line <= 9; line++)
            {
                ...
                foreach (var position in board.GetLine(line))
                {
                    var disabled = position.HasNumber ? "disabled" : "";
                    var val = position.HasNumber
                        ? position.ToString()
                        : ToReadableString(position.GetPossibilities());

                    tr.Append(
                        $"<td><input id='cell-{counter++}' data-possibilities={val.Contains(" ")}  type='text' value='{val}' {disabled}></td>");
                }
